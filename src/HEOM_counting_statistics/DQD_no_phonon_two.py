...
def exciton_relaxation_rate(freq, beta, drude_reorg_energy, drude_cutoff_freq):
...
for i,B in enumerate(beta_values):
    for j,E in enumerate(bias_values):
        ...
        if E < 0:
            ...
            AL_plus = np.eye(3)
            AL_plus[1:,1:] = l[0].conj() * r[0] * np.outer(l, r.conj())
            AR_plus = np.eye(3)
            AR_plus[1:,1:] = l[1].conj() * r[1] * np.outer(l, r.conj())
            
            AL_minus = np.eye(3)
            AL_minus[1:,1:] = l[0] * r[0].conj() * np.outer(r, l.conj())
            AR_minus = np.eye(3)
            AR_minus[1:,1:] = l[1] * r[1].conj() * np.outer(r, l.conj())
            
            FL_plus = os.incoherent_super_operator([(AL_plus, 1.)])
            FR_plus = os.incoherent_super_operator([(AR_plus, 1.)])
            FL_minus = os.incoherent_super_operator([(AL_minus, 1.)])
            FR_minus = os.incoherent_super_operator([(AR_minus, 1.)])
            #print FL_plus + FR_plus
            ...
            AL_plus = np.eye(3)
            AL_plus[1:,1:] = l[0] * r[0].conj() * np.outer(r, l.conj())
            AR_plus = np.eye(3)
            AR_plus[1:,1:] = l[1] * r[1].conj() * np.outer(r, l.conj())
            
            AL_minus = np.eye(3)
            AL_minus[1:,1:] = l[0].conj() * r[0] * np.outer(l, r.conj())
            AR_minus = np.eye(3)
            AR_minus[1:,1:] = l[1].conj() * r[1] * np.outer(l, r.conj())
            
            FL = os.incoherent_super_operator([(AL_plus, 1.)])
            FR = os.incoherent_super_operator([(AR_plus, 1.)])
            #print FL+FR
        ...
        down_rate = exciton_relaxation_rate(-delta, B, drude_reorg_energy, drude_cutoff)
        up_rate = exciton_relaxation_rate(delta, B, drude_reorg_energy, drude_cutoff)
#         print down_rate
#         print up_rate
