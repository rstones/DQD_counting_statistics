...
data = np.load('data/F2_reorg_energy_heom_data_K9_vals_'+str(re_min)+'-'+str(re_max)+'.npz')
...
def calculate_FCS(reorg_energy):
    ...
    data_pt = np.load('data/F2_reorg_energy_heom_data_K9_vals_'+str(re_min)+'-'+str(re_max)+'_'+str(reorg_energy)+'.npz')
...
np.savez('data/F2_reorg_energy_data_K9_vals_'+str(re_min)+'-'+str(re_max)+'.npz', reorg_energy_values=reorg_energy_values, mean_heom=results.T[0], \
